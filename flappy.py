...
last_pipe = pygame.time.get_ticks() - pipe_frequency   # condition met right away
score = 0
pass_pipe = False
...
class Bird(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if game_over == False:
            # jump
            if pygame.mouse.get_pressed()[0] == 1 and self.clicked == False: # left click has happened
                self.clicked = True
                self.vel = -9
            if pygame.mouse.get_pressed()[0] == 0:  # left click not happened
                self.clicked = False
            ...
            if self.counter > flap_cooldown:   # how quickly animation moves
                self.counter = 0
                self.index += 1
...
while run:
    ...
    screen.blit(bg, (0,0))
    # draws second background since png small
    screen.blit(bg, (360, 0))
    ...
    screen.blit(ground_img, (ground_scroll + 336, 640))
    ...
    if pygame.sprite.groupcollide(bird_group, pipe_group, False, False) or flappy.rect.top < 0:
        game_over = True
    # delete if collided with booleans
